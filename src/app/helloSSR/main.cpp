...
void Do_Movement();
void renderScene( const common::Shader &shader);
void renderQuad();
void renderCube();
...
unsigned int cameraDepthFBO[3] = {0 , 0, 0};
unsigned int cameraDepthMap[3] = { 0, 0, 0 };
...
bool debugDepthKeyPressed = false;
...
unsigned int depthFBO0;
unsigned int depthFBO1;
unsigned int depthFBO2;
...
void Do_Movement()
{
	...
	if (keys[GLFW_KEY_Q] && !debugDepthKeyPressed)
	{
		debugDepthMapIndex = (debugDepthMapIndex + 1) > 2 ? 0 : (debugDepthMapIndex + 1);
		debugDepthKeyPressed = true;
	}
	if (!keys[GLFW_KEY_Q])
	{
		debugDepthKeyPressed = false;
	}
